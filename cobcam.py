...
class MyHTMLParser(HTMLParser):
    ...
    def handle_data(self, data):
        if self.counter == 2:
            ...
            femails.close()
            self.counter = -1
        if self.counter == 1 and data[0] == 'N' and data[1] == 'ã' and data[2] == 'o' and data[3] == ' ' and data[4] == 'i':
            self.counter = 2
...
#MB
school = 1
fbody = open("Text\\Cam MB Fixed.txt", "r",encoding='utf8')
for line in fbody:
    parser = MyHTMLParser()
    parser.feed(line)
fbody.close()
...
#MBF
school = 2
fbody = open("Text\\Cam MBF Fixed.txt", "r",encoding='utf8')
for line in fbody:
    parser = MyHTMLParser()
    parser.feed(line)
fbody.close()
